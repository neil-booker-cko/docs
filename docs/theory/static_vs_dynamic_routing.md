...
1. **Next-hop:** The IP address of the next router or the outbound interface
1. **Administrative Distance (AD):** A preference metric (0–255; lower is preferred)
1. **Metric:** Optional metric used for tie-breaking if multiple routes to the same
   destination exist
...
1. **CPU efficiency:** No routing protocol computations. Even ancient routers can handle
   static routes.

1. **Security:** Routes are not leaked via protocol advertisements. An attacker cannot
   inject false routes via a routing protocol.

1. **Bandwidth efficiency:** No periodic hello messages or routing updates. Minimal
   overhead.

1. **Debugging simplicity:** A static route either works (next-hop is reachable) or does
...
1. **Manual updates required:** Any topology change (new site, new link) requires manual
   route configuration. Does not scale.

1. **No redundancy awareness:** Static routes cannot automatically switch to a backup path
   if the primary path fails.

1. **Operator error:** Large static route tables are error-prone. Typos in CIDR notation or
   next-hop IP addresses go undetected until traffic is affected.

1. **Not suitable for complex topologies:** Hundreds of static routes are unmanageable.
...
1. **Topology exchange:** Routers advertise their connected interfaces and learned routes
   in protocol-specific messages (LSAs in OSPF, distance vectors in EIGRP, etc.).

1. **Route computation:** Each router computes shortest paths (or best paths) based on
   received topology information.

1. **Convergence:** Once all routers have exchanged information and recomputed their
   tables, the network is said to have **converged**. Traffic flows along optimal paths.

1. **Adaptation:** When a link fails, routers detect the failure (hello timeout) and
...
1. **Scalability:** Routers learn routes from many peers. The number of routes grows
   automatically with network size.

1. **Optimisation:** Routers choose paths based on metrics (cost, delay, bandwidth). The
   "best" path is chosen automatically.

1. **Redundancy-aware:** If multiple paths exist, dynamic routing can load-balance traffic
   across them (ECMP) or pre-compute backup paths (EIGRP).

1. **Minimal manual configuration:** Routes are not manually entered per router. A single
...
1. **Convergence time:** If a link fails, convergence can take seconds to minutes (default
   timers). During convergence, traffic may be black-holed or misrouted.

1. **CPU overhead:** SPF computation (OSPF) or diffusing computations (EIGRP) consume CPU,
   especially during convergence on large networks.

1. **Bandwidth overhead:** Periodic hello messages and routing updates consume bandwidth,
   though typically minimal on modern networks.

1. **Route leakage:** A misconfigured routing protocol can advertise unintended routes to
   peers, causing traffic redirection or loops.

1. **Debugging difficulty:** Troubleshooting routing protocol issues requires understanding

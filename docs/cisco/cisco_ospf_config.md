...
- **Router ID (RID):** OSPF selects the highest IP address on an active loopback
interface,

interface,

or the highest physical interface IP if no loopbacks exist. Always set it explicitly
with
...
router ospf 1
 ...
 auto-cost reference-bandwidth 10000
area 1 stub                          ! Must match ABR; no Type 5 LSAs; uses default
route
```

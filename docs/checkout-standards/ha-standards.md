...
All services that originate traffic from the FortiGate — syslog, NetFlow, RADIUS, and TACACS+
— use the cluster management VLAN interface IP as `source-ip`. This is the shared IP on the
management VLAN interface (e.g., `bond0.701`). It is synced to both nodes via HA config sync
and follows the active device on failover, so remote servers always see a single, stable source
address. No per-node source IP configuration is needed for these services.
...
**Standard:** NetFlow is generated by the active device only. Set `source-ip` to the cluster's
shared management VLAN interface IP. This IP is synced to both nodes via HA config sync and
follows the active device on failover — the collector always sees the same source address.
...
config system netflow
    ...
    config collectors
        edit 1
            ...
            set source-ip "<CLUSTER_MGMT_VLAN_IP>"
        next
    end
end
```

Enable the NetFlow sampler on the management VLAN interface and any data interfaces that should
export flows:

```fortios
config system interface
    edit "<MGMT_VLAN_INTERFACE>"
